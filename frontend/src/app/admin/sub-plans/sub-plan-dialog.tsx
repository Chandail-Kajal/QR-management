...
export function SubPlanDialog({
  ...
  const schema =
    mode === "create" ? createSubPlanYupSchema : updateSubPlanYupSchema;
  ...
  useEffect(() => {
    if (open) {
      ...
      reset({
        ...
        allowPasswordProtection:
          initialValues?.allowPasswordProtection ?? false,
      });
    }
  ...
  const handleNext = async () => {
    const fieldsToValidate = STEPS[currentStep - 1]
      .fields as unknown as (keyof CreateSubPlanValues)[];
