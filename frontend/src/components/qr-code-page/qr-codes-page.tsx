        ...
        <div className="flex justify-end opacity-0 group-hover:opacity-100 transition-opacity duration-150">
          <QRActionsDropdown
            onAnalytics={() =>
              router.push(
                folderName
                  ? `/admin/folders/${folderName}/analytics/${qr.id}`
                  : `/admin/qr-codes/${qr.id}/analytics`,
              )
            }
            qr={qr}
            onEdit={onEdit}
        ...
        bottomAddon={
          <SegmentedControl
            ...
            options={[
              ...
              ...(typeCountArray?.map((ob) => ({
                label: (
                  <SegmentLabel
                    label={ob.type}
                    count={ob.count}
                    isActive={type === ob.type}
                  />
                ),
                value: ob.type,
...
const SegmentLabel = (props: {
  ...
  return (
    <div className="flex w-full flex-row gap-1 justify-between items-center px-3">
      {props.label}
      <span
        className={`h-5 w-5  flex items-center justify-center text-xs rounded-full bg-purple-400 text-white ${props.isActive ? " font-semibold" : "font-thin"}`}
      >
        {props.count}
      </span>
    </div>
