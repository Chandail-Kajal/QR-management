...
export function FoldersPage({ userId }: { userId?: string }) {
  ...
  useEffect(() => {
    ...
    if (!userId) {
      ...
      crumbs.push({
        ...
        href: `/admin/users/${userDetails?.name}/folders`,
      });
    }
    console.log(crumbs);
    setBreadcrumbs(crumbs as BreadcrumbItem[]);
  }, [setBreadcrumbs, userDetails]);
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3.5 mb-7 font-bold">
          {folders.map((folder) => (
            <FolderCard
              ...
              onForward={() => {
                if (userId) {
                  router.push(`/admin/users/${userId}/folders/${folder.name}`);
                } else {
